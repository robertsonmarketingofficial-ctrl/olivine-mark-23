...
import {
  ...
  CATEGORIES, STAGES, useToast
} from '../components/shared'

const LOVABLE_PROMPT = `Build me a professional website for a local business.

Pages needed:
- Home page with hero section, tagline, and clear call-to-action ("Call Now" / "Get a Free Quote")
- Services page listing what they offer with icons or cards
- About page with a short story about the business
- Contact page with phone number, email, and a simple contact form

Design direction:
- Clean, modern, mobile-first layout
- Professional colour scheme
- Prominent phone number in the header on every page
- Click-to-call phone number link

Key features:
- Google Maps embed on contact page
- Fast loading, SEO-friendly headings
- Make it look like a $5,000 website

I will give you the business name, phone, and services once you confirm you understand.`

const EMAIL_TEMPLATE = (lead) => ({
  subject: `Quick question about your online presence`,
  body: `Hi there,

I came across ${lead?.name || '[Business Name]'} on Google and wanted to reach out.

I help local businesses in the area get more customers through professional websites — most of my clients see results within the first month.

I noticed ${lead?.websiteSignal === 'No website' ? "you don't have a website listed" : lead?.websiteSignal === 'Social only' ? "you're only running a social page" : "there might be room to improve your online presence"}, and thought it could be worth a quick chat.
...
function FinderTab({ addToPipeline, searchHistory, setSearchHistory, showToast }) {
  ...
  const openResearch = (lead) => { window.open(lead.website || `https://www.google.com/search?q=${encodeURIComponent(lead.name)}`, '_blank') }
      ...
      {!searching && leads.map(lead => (
        <div key={lead.id} className="card fade-up" style={{ padding: '14px 18px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => openResearch(lead)} className="btn btn-ghost btn-sm">🌐 Website</button>
...
function WorkspaceTab({ pipeline, showToast }) {
  ...
  const openLead = (lead) => { setSel(lead) }
  const openEmails = (lead) => { setModal('emails') }
  const openLovable = (lead) => { setModal('lovable') }
        ...
        {sel && (
          <div className="card card-p fade-up" style={{ position: 'sticky', top: 20, maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => window.open(sel.website || `https://www.google.com/search?q=${encodeURIComponent(sel.name)}`, '_blank')} className="btn btn-ghost btn-sm">🔍 Research</button>
...
function PipelineTab({ pipeline, savePipeline, showToast }) {
  ...
  const openBulk = () => {
    setBulkEmail({ subject: 'A quick question about your business', body: 'Hi [BUSINESS_NAME],\n\nI came across your listing on Google and wanted to reach out.\n\nI help local businesses get more customers through professional websites — most clients see results within the first month.\n\nWould you be open to a quick 10-minute call this week?\n\nCheers,\nCallum Robertson\nRobertson Marketing\n0405 866 392\nrobertsonmarketingofficial@gmail.com' })
    setModal('bulk')
  }
        ...
        <div key={lead.id} className="card lead-card" style={{ padding: '16px 20px', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => window.open(lead.website || `https://www.google.com/search?q=${encodeURIComponent(lead.name)}`, '_blank')} className="btn btn-ghost btn-sm">🔍</button>
...
function OutreachTab({ pipeline }) {
  ...
  const genEmails = (lead) => {
    setSel(lead)
    const t = EMAIL_TEMPLATE(lead)
    setEmails({ email1: t, email2: { subject: 'Following up — ' + (lead.name || 'your business'), body: `Hi again,\n\nJust following up on my last message about improving ${lead.name || 'your'} online presence.\n\nHappy to show you a quick demo — no cost, no pressure.\n\nCheers,\nCallum Robertson\n0405 866 392` } })
  }
  const genBulk = () => {
    setBulkEmail({ subject: 'Quick question about your online presence', body: `Hi [BUSINESS_NAME],\n\nI came across your listing on Google and wanted to reach out.\n\nI help local businesses get more customers through professional websites.\n\nWould you be open to a 10-minute call?\n\nCheers,\nCallum Robertson\n0405 866 392\nrobertsonmarketingofficial@gmail.com` })
  }
  }

  return (
    <div>
      <PageHeader title="Outreach & Email" sub="Personalised email templates for every lead" />
      <div style={{ display: 'grid', gridTemplateColumns: sel ? '1fr 1fr' : '1fr', gap: 16 }}>
        <div>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12 }}>
            ...
            <button onClick={genBulk} disabled={pipeline.length===0} className="btn btn-ghost btn-sm">✉ Bulk Template</button>
          </div>
...
function SMSTab({ pipeline, showToast }) {
  ...
  const genSMS = (lead) => {
    setSel(lead)
    setSmsList([
      `Hey, it's Callum from Robertson Marketing. I noticed ${lead.name} doesn't have a website — I help local businesses get one up quick. Worth a chat? 0405 866 392`,
      `Hi there! Callum here — I build websites for local businesses in your area. Noticed ${lead.name} could do with one. Happy to show you a free demo? Reply or call 0405 866 392`,
      `G'day! Callum from Robertson Marketing. I help tradies and local businesses get more work online. Got 10 mins for a quick call? ${lead.phone ? `You can reach me on 0405 866 392` : 'Reply here anytime'}`
    ])
  }
...
function ProposalsTab({ pipeline }) {
  ...
  const gen = (lead) => {
    setSel(lead)
    setProposal(`Subject: Professional website for ${lead.name}

Hi there,

I came across ${lead.name} on Google and wanted to reach out about your online presence.

I'm Callum from Robertson Marketing — I help local businesses in your area get professional websites that actually bring in new customers.

Here's what I offer:

STARTER — $1,500 one-off
...
GROWTH — $1,500 + $350/month
✓ Everything in Starter
✓ Local SEO (show up on Google searches)
✓ Monthly performance reports

FULL SERVICE — $1,500 + $750/month
✓ Everything in Growth
✓ Meta Ads management (Facebook & Instagram)
✓ Leads tracked and reported monthly

Most of my clients get their investment back within the first 2–3 new jobs.

I'd love to show you a quick demo — no cost, no obligation.
...
function ScoringTab({ pipeline }) {
  ...
  const explain = (lead) => { setSel(lead) }
  return (
    <div>
          ...
          {sel&&(
            <div className="card card-p fade-up">
              ...
              <div style={{fontSize:13,color:'var(--text2)',lineHeight:1.8}}>
                {sel.websiteSignal==='No website'&&<div>🔴 No website found (+40 pts)</div>}
                {sel.websiteSignal==='Social only'&&<div>🟠 Social page only (+25 pts)</div>}
                {sel.websiteSignal==='Basic builder'&&<div>🟡 Basic website builder (+15 pts)</div>}
                {sel.websiteSignal==='Has website'&&<div>🟢 Has a real website (+5 pts)</div>}
                {sel.reviewCount<20&&<div>📊 Low review count (+5 pts)</div>}
                <div style={{marginTop:8,color:'var(--text3)'}}>Base score: 50 pts</div>
              </div>
            </div>
          )}
        </div>
...
function EmailModal({ lead, onClose }) {
  const e1 = EMAIL_TEMPLATE(lead)
  const e2 = { subject: 'Following up — ' + (lead?.name || 'your business'), body: `Hi again,\n\nJust wanted to follow up on my last message about your online presence.\n\nI've helped businesses just like ${lead?.name || 'yours'} get more enquiries through a professional website. Happy to show you a quick demo — no cost, no pressure.\n\nCheers,\nCallum Robertson\n0405 866 392` }
  return (
    <Modal title={`Outreach emails — ${lead?.name}`} onClose={onClose}>
      {[{key:'email1',label:'Email 1 — Website pitch',email:e1},{key:'email2',label:'Email 2 — Follow-up',email:e2}].map(({key,label,email})=>(
        <div key={key} style={{marginBottom:20}}>
          ...
          <div style={{background:'rgba(255,255,255,0.04)',borderRadius:7,padding:'7px 10px',marginBottom:6,fontSize:12}}><strong style={{color:'var(--text3)'}}>Subject:</strong> {email.subject}</div>
          <div style={{background:'rgba(255,255,255,0.03)',borderRadius:8,padding:'12px',fontSize:13,color:'var(--text)',lineHeight:1.7,whiteSpace:'pre-wrap'}}>{email.body}</div>
        </div>
      ))}
    </Modal>
...
function LovableModal({ lead, onClose }) {
  return (
    <Modal title={`Lovable.dev prompt — ${lead?.name || 'Business'}`} onClose={onClose}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
        <p style={{margin:0,fontSize:13,color:'var(--text3)'}}>Copy this prompt and paste it into <a href="https://lovable.dev" target="_blank" rel="noreferrer" style={{color:'var(--olive2)'}}>lovable.dev</a> — then type the business name when asked.</p>
        <CopyBtn text={LOVABLE_PROMPT} label="Copy prompt"/>
      </div>
      <div style={{background:'rgba(255,255,255,0.03)',borderRadius:10,padding:'14px',fontSize:13,color:'var(--text)',lineHeight:1.7,whiteSpace:'pre-wrap',maxHeight:400,overflowY:'auto'}}>{LOVABLE_PROMPT}</div>
      <div style={{marginTop:14,display:'flex',gap:10}}>
        <a href="https://lovable.dev" target="_blank" rel="noreferrer" className="btn btn-primary btn-sm" style={{textDecoration:'none'}}>Open Lovable.dev →</a>
      </div>
    </Modal>
  )
}
...
function SettingsTab() {
  return (
    <div>
      <PageHeader title="Settings & Debug" sub="Environment info and setup guide" />
      <div className="card card-p" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>Setup Guide</div>
        {[
          { n: '1', t: 'Add your Google Places API key', d: 'Go to Vercel project → Settings → Environment Variables. Add GOOGLE_PLACES_API_KEY to enable the Lead Finder.' },
          { n: '2', t: 'Get your API key', d: 'Visit console.cloud.google.com → enable Places API → create a key.' },
          { n: '3', t: 'Redeploy after adding', d: 'Vercel requires a redeploy after adding environment variables. Go to Deployments → three dots → Redeploy.' },
        ].map(s => (
